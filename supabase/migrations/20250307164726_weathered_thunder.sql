/*
  # Fix T20 Blast Match IDs and Add Tournament Data
...
-- Create a sequence for match IDs
CREATE SEQUENCE IF NOT EXISTS t20_blast_2025_match_seq START 1;

-- Insert North Group matches
WITH north_matches AS (
  SELECT 
    2025003000 + nextval('t20_blast_2025_match_seq') as id,
...
SELECT * FROM north_matches;

-- Insert South Group matches
WITH south_matches AS (
  SELECT 
    2025003000 + nextval('t20_blast_2025_match_seq') as id,
    2025003 as tournament_id,
    t1.name || ' vs ' || t2.name as name,
